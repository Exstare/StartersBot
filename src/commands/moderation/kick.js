...
module.exports = class Command extends BaseCommand {
    constructor() {
        super('kick', 'moderation', [], 'kick', 'KICK_MEMBERS');
    }

    async run(client, message, args) {
        
        //Check if there is an argument (the user to kick)
        if (!args[0]) return failEmbed(message, `No user given!`);

        //Get the person to kick
        ...
        //Kick the user
        await person.kick(reason).catch(error => errorEmbed(message, `Oh no, something went wrong!\n\`\`\`${error}\`\`\``));

        //Send the confirmation
        doneEmbed(message, `${person} has been kicked!`);
